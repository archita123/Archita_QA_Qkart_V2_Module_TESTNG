...
public class Home {
    ...
    public Boolean searchForProduct(String product) {
        try {
            ...
            WebDriverWait wait = new WebDriverWait(driver,30);
             ...
             wait.until(ExpectedConditions.or(ExpectedConditions.presenceOfElementLocated(By.xpath("//p[contains(text(), '"+product+"')]")),
    ...
    public Boolean addProductToCart(String productName) {
     ...
     //Iterate through each product on the page to find the WebElement corresponding to the matching productName
      // Click on the "ADD TO CART" button for that element,  // Return true if these operations succeeds
    ...
    // Increment or decrement the quantity of the matching product until the current // quantity is reached (Note: Keep a look out when then input quantity is 0,
// here we need to remove the item completely from the cart)  // Find web element corresponding to each of the cart items
